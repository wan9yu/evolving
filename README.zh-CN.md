...
ev verify                                 # 重查锚的解析；报告漂移
ev pause                                  # 人的每日仪式：要证据、附证据、灰置、放手
...
证据指针类型：`commit:<sha>` · `test:<路径>[::<匹配行>]` · `file:<路径>[::<匹配行>]` ·
`artifact:<名字>[::<匹配行>]` · `metric:<文本>` 和 `url:<文本>`（仅记录，不检查）。
...
判断发生在暂停里：要证据、当场附上、灰置、或者放手让它死。闭合是一个独立而刻意的动作
——`ev close <id>`，给配得上的声称。攒下来的是线——两个裸计数，永远没有分数。
