...
`ev` 就是弥合这道缝隙的那一层。它把人类亲自做出的决策*以及它所依据的根据*记录为一条不可变、内容寻址的链，为根据绑定一个可证伪的检查，并在**那个检查变红时让决策重新浮现——在你下一次 `ev check`，无需常驻守护进程。** 它处理的是事实，而非裁决：没有评分，没有排名，没有自动判决——只有一份诚实的记录，写明决定了什么、为什么、谁在承担责任，以及守护每条理由的检查是否仍存活。底层是*决策的 git*；agents 提议（propose），由人类批准（ratify）并承担责任。
...
- **一份 ADR。** 一份架构决策记录（Architecture Decision Record）捕获一个决策*为什么*被做出——以散文形式，写一次，然后任其腐烂。没人会重读它，也没有任何东西在它所依据的前提不再成立时告诉你。`ev` 同样记录*为什么*，但把它绑定到一个可证伪的检查上，并在**那个检查变红时把决策带回来**。一份 ADR 是一块墓碑；一个 `ev` 决策是活着的。用 AI 自动起草这份 ADR、再放在人工评审门后，并不改变这一点——那只是一块*更快*的墓碑；仍然没有任何东西把它和"它的前提是否还成立"连起来。
...
- **一个"PR 上的决策提醒"，或一个架构适应度检查。** 有些工具在一个 pull request 触及某些文件时把相关决策浮现出来，或在代码违反某条决策定下的规则时让 CI 失败。两者都对一个*代码事件*触发——某个文件被动了、某条规则被违反了。`ev` 的触发不同：**那个决策的前提不再为真**——这可能在没有任何规则被违反、没有任何被守护文件被触及时发生，反过来一个被触及的文件也可能让前提依然成立。`ev` 把它的检查绑在*理由*上，而不是绑在文件路径或一条 lint 规则上。
...
- **它假设每个 store 单一写者。** `ev` 在追加一个 tick 并推进 `HEAD` 时不加锁，所以两个 `ev` 进程并发写同一个 store 可能分叉链、或竞争指针。这是有界且可恢复的——内容寻址防止损坏——但如果你用脚本调用 `ev`，请把写者串行化。
