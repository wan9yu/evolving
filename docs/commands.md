...
thereafter shown "ratified by `<child>`". Same mint-a-child mechanics as [`ev supersede`](#ev-supersede);
...
### Tag discrepancies on re-import (resolved with `ev supersede`)
...
The remedy is **[`ev supersede`](#ev-supersede)**: it appends a corrective child carrying the
...
- backfill (stdout): `<(dry-run) >imported N, skipped M, re-linked K, J source-only gap(s)`
  ...
  is appended **only when** `D > 0` tag discrepancies were surfaced (see [above](#tag-discrepancies-on-re-import-resolved-with-ev-supersede)).
- per discrepancy (stderr, one line each): `discrepancy: source <key> (tick <id>): <tag stored=… incoming=…; …> — NOT applied (ticks are immutable; resolve with \`ev supersede <id>\`)`.
...
**Synopsis:** replace a prior ruling under `ev`'s append-only law. It **never** rewrites the target
...
- **RE-TAG** — `ev supersede <id>` with **only tags**, no new ruling: copies the target's hashed payload
  (`decision` / `observe` / `grounds`) verbatim — recognizably the same decision — and carries a
  corrected non-hashed tag (`authority` / `jurisdiction` / `provenance`). *Write the id; don't restate
  the ruling.*
- **OVERTURN** — `ev supersede <id> "<new ruling>"`: a **fresh decision** (the `ev decide` grammar for
  its grounds) that replaces the prior ruling. A supersession must say **why** the prior ruling no
  longer holds, so at least one `--assume` is **required**.
...
| --- | --- | --- | --- |
| `--assume` | `"<why>"` | overturn (**required**, repeatable) | Why the prior ruling no longer holds — a chosen ground of the new ruling. |
| `--reject` | `"<option>: <why>"` | overturn (repeatable) | A road not taken (same `<option>: <why>` form as `ev decide`). |
| `--authority` | `user-ruled` \| `agent-disposable` | both | The standing. Out-of-vocabulary is refused. |
| `--jurisdiction` | `A` \| `B` \| `C` \| `D` | both | The decision class. Out-of-vocabulary is refused. |
| `--provenance` | `imported` \| `agent-proposed` \| `human-now` | re-tag | The corrected provenance. |
| `--source-ref` | a key | overturn | An opaque producer / work-unit key for the new ruling. |
| `--blame` | a name | no* | The author. *If omitted, falls back to `git config user.name`; one must resolve to a non-empty name (R5). A supersession is a human-authored act.* |
...
- **Re-tag needs a tag.** No new ruling AND no tag → `ev supersede (re-tag) needs at least one of
  --authority / --jurisdiction / --provenance — or pass a new ruling text to overturn`.
- **Overturn needs a reason.** A new ruling with no `--assume` → `ev supersede <id> <new ruling> needs
  at least one --assume <why the prior ruling no longer holds>`.
- **No-op re-tag.** Every supplied tag already holds that value → `tick <id> already carries those tags
  — nothing to re-tag`.
- **Unknown id.** An `id` not in the store → `no such tick: <id>` (re-tag) / `no such tick to supersede:
  <id>` (overturn).
- **Vocabulary** and the **detect-only structural lock** (a `C` / `D` jurisdiction may carry no runnable
  Test) are enforced with the same strings `ev decide` uses.
- **No store.** → `no .evolving/ store here — run \`ev init\` first`.
...
- re-tag success: `re-tagged <child> (supersedes <id>)` — no ground count (it copies the parent's
  grounds verbatim, so a count would be a zero-entropy echo; a count appears only at creation).
- overturn success: `recorded <child> (<n> ground(s)) — supersedes <id>`.
- failure (stderr): `error: <message>`.
...
# ev reopen <child>  now marks it "superseded by <new>"
```

A migrate **discrepancy** (a re-import whose resolved tags differ from the stored tick — see
[`ev migrate`](#tag-discrepancies-on-re-import-resolved-with-ev-supersede)) is resolved by the **re-tag**
branch.
...
lineage to its current state** (an [`ev supersede`](#ev-supersede) child supersedes the stale tick it
...
- declared tags, each only when present (stdout): `authority: <value>`, then
  `jurisdiction: <value>`, then `source_ref: <value>`, then `supersedes: <id>` (the
...
(an [`ev supersede`](#ev-supersede) child supersedes the stale tick it re-tags — so a ruling whose
...
**current** state — an [`ev supersede`](#ev-supersede) child supersedes the stale tick it re-tags, so
