...
#[test]
fn compute_id_should_freeze_the_rejected_road_tripwire_id_when_a_rejected_ground_carries_a_check() {
    // given: the case1 lineage/text, but the rejected:Redis road now CARRIES a Check::Test — the
    // 0.1.8 tripwire byte layout (a rejected: ground WITH a check, which no other golden exercises)
    let mut t = book(
        "7b21f0a4c8de",
        "multi-pod restore-safety counter — chat-room R2289→R2290",
        "restore-safety counter DB-backed; reject Redis",
    ...
    // authority is non-hashed (it must NOT move the id); set it for fidelity to a real tripwire tick
    t.authority = Some("user-ruled".into());

    // when: its canonical id is computed
    let id = compute_id(&t);

    // then: it matches the frozen tripwire golden id (a check on a rejected: ground hashes
    // identically to one on a chosen ground; authority/provenance never enter the payload)
    assert_eq!(id, "9c5feb4582ac");
}
